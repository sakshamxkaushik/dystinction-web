  ...
  return (
    <section className="w-full max-w-8xl xl:max-w-8xl mx-auto px-4 sm:px-10 lg:px-12">
      ...
      <div className="relative w-full max-w-5xl mx-auto px-3 sm:px-6 text-center z-10 flex flex-col items-center space-y-8 md:space-y-12 lg:space-y-16">
        {/* Trust indicators */}
        <div className={`flex justify-center mb-2 sm:mb-4 transition-all duration-500 delay-100 ${
        ...
        <div className="hero-container floating-element pt-24 sm:pt-32 md:pt-20">
          <div className="flex flex-col items-center text-center space-y-4 sm:space-y-6 md:space-y-8">
            ...
            <h1 
              id="hero-title"
              className={`hero-title text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold leading-snug sm:leading-tight md:leading-tight max-w-xl sm:max-w-3xl md:max-w-5xl mx-auto transition-all duration-600 delay-0 ${
                isVisible 
            ...
            {/* Subtitle */}
            <p className={`hero-subtitle text-base sm:text-lg md:text-xl lg:text-2xl text-muted-foreground max-w-sm sm:max-w-2xl md:max-w-3xl mx-auto leading-relaxed transition-all duration-500 delay-200 ${
              isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
            }`}>
          ...
          <div className={`absolute inset-0 -z-10 transition-opacity duration-600 delay-100 ${
        ...
        {/* CTA Buttons */}
        <div className={`flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center w-full max-w-xs sm:max-w-none mx-auto mb-8 md:mb-12 lg:mb-16 transition-all duration-500 delay-300 ${
          isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
        }`}>
          <Button
            variant="liquid"
        ...
        {/* Incubator logos */}
        <GlowingStarsBackgroundCard className={`relative bg-glass w-full max-w-xs xs:max-w-md sm:max-w-2xl md:max-w-3xl lg:max-w-4xl rounded-2xl mt-2 sm:mt-4 overflow-hidden transition-all duration-500 delay-400 ${
          isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
        }`}>
          {/* Overlay content */}
        ...
        {/* Floating elements */}
        <div className={`hidden sm:block absolute top-1/4 left-10 w-4 h-4 bg-primary/30 rounded-full blur-sm float z-0 transition-opacity duration-500 delay-500 ${
          isVisible ? 'opacity-100' : 'opacity-0'
        }`} />
        <div className={`hidden sm:block absolute bottom-1/3 right-10 w-6 h-6 bg-secondary/30 rounded-full blur-sm float-delay z-0 transition-opacity duration-500 delay-600 ${
          isVisible ? 'opacity-100' : 'opacity-0'
        }`} />
        <div className={`hidden sm:block absolute top-1/2 left-1/3 w-3 h-3 bg-primary/40 rounded-full blur-sm float z-0 transition-opacity duration-500 delay-700 ${
          isVisible ? 'opacity-100' : 'opacity-0'
        }`} />
      </div>
    </section>
